...
import { RecipientListModal } from '@/app/(dashboard)/cong-van/di/recipient-list-modal'
import { StatusDetailsModal } from '@/app/(dashboard)/cong-van/di/status-details-modal'
import { SentDocumentActions } from '@/app/(dashboard)/cong-van/di/sent-document-actions'
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export function SentDocumentRow({ doc, currentUserId, isITAdmin }: { doc: any, currentUserId?: string, isITAdmin?: boolean }) {
  ...
  const handleRowClick = (e: React.MouseEvent) => {
    ...
    router.push(`/cong-van/di/${doc.id}?from=sent`)
  }
